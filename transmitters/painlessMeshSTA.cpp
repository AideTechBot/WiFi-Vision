...
void ICACHE_FLASH_ATTR StationScan::stationScan() {
    ...
    //ssid = "Pronto Vision Demo";
    ssid.toCharArray(tempssid, ssid.length() + 1);

    //scanConfig.ssid = (uint8_t *) tempssid; // limit scan to mesh ssid
    scanConfig.bssid = 0;
    scanConfig.channel = 1;//mesh->_meshChannel; // also limit scan to mesh channel to speed things up ...
...
void ICACHE_FLASH_ATTR StationScan::scanComplete(bss_info *bssInfo) {
    ...
    while (bssInfo != NULL) {
        ...
        if (*((char *)bssInfo->ssid)==*((char*)"Pronto Vision Demo")){
          temp = bssInfo->rssi;
        }
